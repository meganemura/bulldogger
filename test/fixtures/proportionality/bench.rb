...
# Bench harness for the probe proportionality law: probe's cost scales
# with calls to its targeted method (M), not with the total call count
# across the workload (N). Drives Proportionality::App#step
# target_calls + other_calls times, routing to Target#hit exactly
# target_calls times and Other#noise exactly other_calls times,
# interleaved so the probe's TracePoint never sees a long uniform run
# of only one method.
#
# The driving loop lives here, not in app.rb, on purpose: a wrapper
# method around it would itself be a Ruby-level call the probe's
...
# checking the actual evidence probe wrote, not assumed away:
# `Proportionality::App.new` (and the Target/Other it constructs) is
# built before any mode's probing starts, and the drive loop below is
# a `while`, not `Integer#times` -- `#times` is itself a genuine
# Ruby-level method (defined in Ruby's own `<internal:numeric>`
# prelude), so its block form would add one more :call/:return pair
# that has nothing to do with target_calls or other_calls.
#
# ARGV: mode ("baseline" | "probe"), target_calls, other_calls
# Prints:
...
# Built before any mode enables probing, so the constructors themselves
# never appear as events -- see the note above.
