...
# exactly what stresses RaiseTracker's raise-exit discriminator
  ...
  class Runner
    def self.run(node)
      ...
      when "raise"
        raise Boom, "boom"
      when "reraise"
        # rescue then bare `raise`: measured directly that a bare
        # re-raise fires :raise again for the same exception, so
